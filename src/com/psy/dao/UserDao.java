...
public class UserDao {
    ...
    public static int addUser(RegUser user) {
        ...
        QueryHelper.update(runner, connection, SQL.ADD_USER, new String[]{user.getName(), Encipher.encrypt(user.getPassword()), user.getEmail(), User.TYPE_NORMAL});
    ...
    public static User validateUser(LoginUser loginUser) {
        ...
        User user = QueryHelper.queryBean(runner, User.class, null, SQL.VALIDATE_USER, new String[]{loginUser.getEmail(), Encipher.encrypt(loginUser.getPassword())});
        return user;
    }
    ...
    public static int findUserIdByName(String username) {
        QueryRunner runner = new QueryRunner(DBManager.getDataSource());
        User user = QueryHelper.queryBean(runner, User.class, null, SQL.FIND_USERID_BY_USERNAME, username);
        if (user != null) {
            return user.getId();
        }
        return 0;
